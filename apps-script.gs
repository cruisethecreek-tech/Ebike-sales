...
function _doGetInner(e, action) {
  ...
  // ── Ebike invoices + catalog (consolidated from the old "Project C").
  // These reach the ebike Inventory Sheet by ID (see bottom of file). ──
    ...
    'Faqs': {
      ...
      rows: [
        ...
        [37, 'Tours & Rentals', '', 'Do you rent anything besides e-bikes?',
          "Yes — **Jetti walking poles**, if you'd rather explore the bikeway on foot. **Self-serve, no staff needed:** book online, get a locker code by text or email, and unlock the Trailside locker at the **Kirk Road Trailhead** (Kirk Road location only). Four sizes fit heights from 5'0\" to 6'2\" (Small, Medium, Large, Extra Large). Each rental is a full set of two poles. Rates: $8 (1 hr), $12 (2 hr), $15 (3 hr), $18 (4 hr) — same for every size. Booking the night before locks in your size, but walk-ups are welcome (book from your phone at the trailhead). A waiver and a card on file are required. Launching August 2026. [Details & booking](trailside.html#jetti)."],
...
// ════════════════════════════════════════════════════════════
// EBIKE INVOICES + CATALOG (consolidated from the old "Project C")
// All reach the ebike Inventory Sheet by ID via openById, so they run
// correctly even though this project is bound to the CMS content sheet.
// ════════════════════════════════════════════════════════════
